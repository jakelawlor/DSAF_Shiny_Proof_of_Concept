...
#Step 1: Fisher’s Method function
...
calculate_fisher_pairs <- function(df) {
  ...
  fisher_results <- map_dfr(pairs, function(x) {
    p1 <- df[[x[1]]]
    p2 <- df[[x[2]]]
    combined <- combine_fisher(p1, p2)
...
canada_fisher <- calculate_fisher_pairs(Canada.p_wide)
usa_fisher <- calculate_fisher_pairs(USA.p_wide)
head(canada_fisher)

#3. Add some new fields: significance and rankings
#classify the combined p.values
usa_fisher <- usa_fisher %>%
  mutate(
...
usa_fisher$p.significant <- factor(
  usa_fisher$p.significant,
  levels = c("Not significant", "Marginal", "Strong", "Very strong")
)
canada_fisher$p.significant <- factor(
  canada_fisher$p.significant,
  levels = c("Not significant", "Marginal", "Strong", "Very strong")
)
...
usa_fisher$Combined_P<-format(usa_fisher$Combined_P, scientific = FALSE)
canada_fisher$Combined_P<-format(canada_fisher$Combined_P, scientific = FALSE)

#Step 4: Plotting/ Interpretation
library(ggplot2)

# rename the indicators so it plots easier
unique(canada_fisher$Indicator1)
unique(canada_fisher$Indicator2)
...
all_levels <- union(
  unique(na.omit(canada_fisher$Indicator1)),
  unique(na.omit(canada_fisher$Indicator2))
)
canada_fisher$Indicator1 <- factor(canada_fisher$Indicator1, levels = all_levels)
canada_fisher$Indicator2 <- factor(canada_fisher$Indicator2, levels = all_levels)
#some of the relations need to be swapped for plotting
rows_to_swap <- c(6, 8)  # for example

for (i in rows_to_swap) {
  tmp <- canada_fisher$Indicator1[i]
  canada_fisher$Indicator1[i] <- canada_fisher$Indicator2[i]
  canada_fisher$Indicator2[i] <- tmp
}

ggplot(canada_fisher, aes(x = Indicator1, y = Indicator2, fill = p.significant)) +
...
#Plot USA Matrix
usa_fisher$Indicator1 <- factor(usa_fisher$Indicator1, levels = all_levels)
usa_fisher$Indicator2 <- factor(usa_fisher$Indicator2, levels = all_levels)
#some of the relations need to be swapped for plotting
rows_to_swap <- c(2, 8)  # for example

for (i in rows_to_swap) {
  tmp <- usa_fisher$Indicator1[i]
  usa_fisher$Indicator1[i] <- usa_fisher$Indicator2[i]
  usa_fisher$Indicator2[i] <- tmp
}

ggplot(usa_fisher, aes(x = Indicator1, y = Indicator2, fill = p.significant)) +
  geom_tile(color = "white") +
  geom_text(aes(label = Rank), size = 4) +
  scale_x_discrete(drop = FALSE) +   # keep all levels
  scale_y_discrete(drop = FALSE) + 
...
#ok so the p.values are really small because the indicators are derived from the same data and correlated
...
combined_p <- EmpiricalBrownsMethod::empiricalBrownsMethod(
  data_matrix = df[, c("Indicator1", "Indicator2")],
  p_values = c(p1, p2)
)
